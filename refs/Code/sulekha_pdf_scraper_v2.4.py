...
BACKOFF_MAX = 180.0          # seconds cap

print("🔹 Running sulekha_table_scraper_v3.py")
...
def make_key(year_val, lb_val, district_index, lb_index):
    # stable unique key independent of name slugs
    return f"{year_val}|{lb_val}|{district_index}|{lb_index}"

def db_get(con, key):
    cur = con.execute("SELECT expected, scraped, status, csv_path FROM progress WHERE key=?", (key,))
    row = cur.fetchone()
    if not row:
        return None
    return {"expected": row[0], "scraped": row[1], "status": row[2], "csv_path": row[3]}

def db_upsert(con, key, csv_path, expected=None, scraped=None, status=None):
    now = datetime.utcnow().isoformat()
    existing = db_get(con, key)
    if existing is None:
        con.execute(
            "INSERT OR REPLACE INTO progress(key, csv_path, expected, scraped, status, updated_at) VALUES (?,?,?,?,?,?)",
            (
                key,
                csv_path,
...
class SulekhaClient:
    ...
    def _request_with_retries(self, method, url, **kwargs):
        """
        Retries on:
        - Connection resets (WinError 10054)
        - requests.ConnectionError/Timeout
        - 5xx responses
        Recreates session occasionally.
        """
        last_exc = None
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                backoff = backoff * (0.7 + random.random() * 0.6)  # jitter 0.7x..1.3x

                code = ""
                if isinstance(e, requests.HTTPError) and getattr(e, "response", None) is not None:
                    code = f"(status={e.response.status_code})"

                print(f"    ⚠ Network/server error {code} on attempt {attempt}/{MAX_RETRIES}: {e}")
                print(f"    ↻ Backing off for {backoff:.1f}s")

                time.sleep(backoff)

                # Occasionally recreate session (helps after connection reset)
                if attempt in {3, 6}:
...
def slugify(s, max_len=80):
    s = s.strip().replace(" ", "_")
    # keep Malayalam? Windows filenames are fine with unicode,
    # but keep it simple: allow unicode letters too
    s = re.sub(r'[<>:"/\\|?*]', "_", s)
    s = re.sub(r"\s+", "_", s)
    if not s:
        s = "name"
...
def get_lb_csv_path(
    year_val, lb_val, district_index, district_name, lb_index, lb_name
):
...
def parse_projects_and_pager_full(soup):
    ...
    Returns:
      ...
      pager: dict with:
        ...
        - ellipsis: list of postbacks where link text == "..."
    """
    ...
    for tr in rows[1:]:
        ...
        # Skip bogus pager-number rows accidentally parsed
...
# ----------------- LOCALBODY SCRAPER (INCREMENTAL CSV + SQLITE) -----------------
...
def safe_reset_to_context(client, year_val, lb_val, district_postback):
    """
    After failures, try to restore state:
...
def scrape_localbody_projects_incremental(
    ...
    # update db with path & expected
    db_upsert(con, key, csv_path, expected=expected)
    ...
    # If PARTIAL exists, we overwrite and restart for correctness (safer).
    # (Could implement page-resume later, but this is robust.)
    ...
    db_upsert(con, key, csv_path, scraped=0, status="PARTIAL")
    ...
    while True:
        ...
        if page_id in visited_pages:
            print(f"          ⚠ Page loop detected at page {page_id}. Stopping this LB.")
            break
        ...
        db_upsert(con, key, csv_path, scraped=rows_written, status="PARTIAL")

        # stop early if we hit expected
        ...
        # Prefer numeric next page if visible; else click ellipsis to reveal more pages.
        pages = pager["pages"]  # {page_num: pb}
        ellipsis = pager["ellipsis"]  # [pb, pb,...]

        # Determine the next page number we want
        ...
        if want in pages:
            ...
            # Not visible; use ellipsis if present, then try again
            if ellipsis:
                # click the last ellipsis usually advances the window
                next_pb = ellipsis[-1]
    ...
    if expected is not None and expected > 0 and rows_written >= expected:
        final_status = "DONE"
    db_upsert(con, key, csv_path, scraped=rows_written, status=final_status)
...
def crawl_tables(
    ...
    try:
        ...
        print("❌ Failed initial base load:", e)
        raise
    ...
    for i, (year_val, year_label) in enumerate(years, start=1):
        ...
        # set year with retries; if it fails, reload base and try again
        try:
            client.postback("drpYear", "", updates={"drpYear": year_val})
        except Exception as e:
            print(f"⚠ Failed to set year {year_label}: {e} -> reloading base and retrying once")
            client.load_base()
            client.postback("drpYear", "", updates={"drpYear": year_val})
        ...
        for j, (lb_val, lb_label) in enumerate(lbtypes, start=1):
            ...
            for dpos, district in enumerate(districts, start=1):
                ...
                try:
                    ...
                    print(f"      ⚠ Failed to enter district {d_name}: {e} -> reset context and continue")
                    try:
                ...
                for lbk, lb in enumerate(lb_rows, start=1):
                    ...
                    # Always restore context to known state before scraping this LB
                    try:
                        client.postback("drpYear", "", updates={"drpYear": year_val})
                        client.postback("drpType", "", updates={"drpType": lb_val})
                        client.postback(d_target, d_arg)
                    except Exception as e:
                        print(f"        ⚠ Failed to restore context before LB: {e} -> reset hard and continue")
                        try:
                            safe_reset_to_context(client, year_val, lb_val, district["postback"])
                        except Exception:
                            pass
                    ...
                    try:
                        ...
                        print("        ↻ Resetting context and continuing with next LB...")
                        try:
                            safe_reset_to_context(client, year_val, lb_val, district["postback"])
                        except Exception:
                            pass
